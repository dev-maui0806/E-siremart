...
export async function registerUserController(request, response) {
  try {
    ...
    // Default avatar path
    const defaultAvatar = path.join(`${process.env.RESEND_API}/assets/images/user_avatar/main.jpg`);
    ...
    console.log(newUser)
    const verifyEmail = await sendEmail(email, name, otp, "email");
    // const verifyEmail = true;
    console.log("verify", verifyEmail);
    
    if (!verifyEmail) {
      return response.status(400).json({
        message: "Failed sending verify Link!",
        error: true,
        success: false,
        data: null,
      });
...
export async function uploadAvatar(req, res) {
  try {
    ...
    const avatarPath = path.join(`${process.env.RESEND_API}assets/images/uploads/`, file.filename);
...
export async function forgotPasswordController(request, response) {
  try {
    ...
    await sendEmail(email, user.name, otp, "forgotPassword");
